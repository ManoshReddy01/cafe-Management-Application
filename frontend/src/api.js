...
export const addMenuItem = async (itemName, price, category) => {
  const res = await fetch(`${BASE_URL}/menu/add`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ itemName, price: parseFloat(price), category }),
  });
  return res.text();
};
...
// ORDERS
export const placeOrder = async (customerName, itemName, quantity, price) => {
...
export const trackOrders = async (customerName) => {
  const res = await fetch(`${BASE_URL}/order/track/${encodeURIComponent(customerName)}`);
  return res.json();
};
...
export const approvePayment = async (orderId) => {
  const res = await fetch(`${BASE_URL}/admin/approve-payment/${orderId}`, {
    method: 'PUT',
  });
  return res.text();
};
...
export const completeOrder = async (orderId) => {
  const res = await fetch(`${BASE_URL}/admin/complete-order/${orderId}`, {
    method: 'PUT',
  });
  return res.text();
};
